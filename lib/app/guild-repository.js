...
/**
 * Saves a character to the guild and remove the corresponding ex-characters if existing.
...
GuildRepository.prototype.saveCharacter = function saveCharacter(character) {
    var self = this;
    return this._saveCharacter(character).then(function() {
        return self._removeExCharacter(character);
    });
};
 ...
 * @returns {object} A promise that resolves once the character has been saved.
 */
GuildRepository.prototype._saveCharacter = function _saveCharacter(character) {
    ...
    return characterReference.once('value').then(function(savedCharacter) {
        if(_.isEmpty(savedCharacter.val())) {
            ...
            return characterReference.set(toInsert);
        }
        else {
            var toUpdate = character.getData();
            return characterReference.update(toUpdate);
        }
    });
};

/**
 * Removes an ex-character from the guild if it exists.
 * @param {object} character The character object that needs its ex-character removed.
 * @returns {object} A promise that resolves once the ex-character has been removed.
 */
GuildRepository.prototype._removeExCharacter = function _removeExCharacter(character) {
    var characterName = character.get('name');
    var exCharacterReference = this.firebase.child('ex-characters/' + characterName);
    return exCharacterReference.once('value').then(function(savedExCharacter) {
        if(!_.isEmpty(savedExCharacter.val())) {
            return exCharacterReference.remove();
        }
    });
};
